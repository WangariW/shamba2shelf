...
export default function BuyerDashboard() {
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      try {
        const buyerRes = await api.get("/buyers/me").catch(() => {
          if (authUser?.id) {
        ...
        if (buyerId) {
          try {
            ...
            setSalesData(calculateSpendingFromOrders(ordersData));
          } catch {
            setOrders([]);
            setSalesData([]); // no fake data
          }
        } else {
          setOrders([]);
          setSalesData([]); // no fake data
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-[#1B1B1B]">
        <p className="text-[#3B1F0E] dark:text-amber-400 text-lg font-medium">
          Loading dashboard...
        </p>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#1B1B1B] text-gray-800 dark:text-gray-200 py-10 px-6 md:px-16 transition-colors duration-300">
      ...
      >
        <div className="w-20 h-20 bg-gray-300 dark:bg-gray-700 rounded-full overflow-hidden mr-6 flex items-center justify-center">
          {buyer?.avatar ? (
            <img
              src={buyer.avatar}
              alt="Avatar"
            ...
            <span className="text-xl font-bold text-gray-800 dark:text-gray-200">
              {getInitials(buyer?.name || authUser?.name)}
            </span>
          )}
        </div>
      ...
      <div className="mb-12">
        ...
        <div className="flex flex-wrap gap-4 mb-4">
          <input
            ...
            className="p-2 rounded-lg bg-gray-100 dark:bg-[#333] dark:text-white placeholder-gray-400 flex-grow"
          />

          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="p-2 rounded-lg bg-gray-100 dark:bg-[#333] dark:text-white"
          >
            <option>All</option>
      ...
      >
        ...
        {salesData.length === 0 ? (
          <p className="text-center text-gray-500 dark:text-gray-400 py-10">
            No spending yet — place your first order to see analytics.
          </p>
        ) : (
      ...
      >
        ...
        {routeError && (
          <p className="text-red-500 text-center">{routeError}</p>
        )}
